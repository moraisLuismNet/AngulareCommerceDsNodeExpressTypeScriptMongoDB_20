...
export class GenresService {
        ...
        next: (response) => {
          ...
          if (Array.isArray(response)) {
            ...
            genres = response.map((item) => ({
              IdMusicGenre:
                item._id || item.IdMusicGenre || item.idMusicGenre || "",
  ...
  addGenre(genre: IGenre): Observable<IGenre> {
    ...
    return new Observable<IGenre>((subscriber) => {
      this.http
        .post<any>(`${this.urlAPI}musicGenres`, requestBody, {
      ...
      this.http
        ...
        .subscribe({
          next: (response) => {
            ...
            const result: IGenre = {
              IdMusicGenre:
                response._id || response.IdMusicGenre || genre.IdMusicGenre,
              NameMusicGenre:
  ...
  deleteGenre(idMusicGenre: number): Observable<IGenre> {
    ...
    return new Observable<IGenre>((subscriber) => {
      this.http
        .delete<any>(`${this.urlAPI}musicGenres/${idMusicGenre}`, { headers })
        .subscribe({
          next: (response) => {
            // Handle different response formats
            const result: IGenre = {
